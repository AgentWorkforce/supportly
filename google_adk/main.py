"""
Customer Service Escalation App — Google ADK

Two Google ADK agents communicate via Relay:
...
config = RelayConfig(
    workspace=os.environ.get("RELAY_WORKSPACE", "customer-service-google-adk"),
...
front_desk_agent = Agent(
    ...
    instruction=(
        ...
        "answer directly and post to the general channel using front_post_to_channel. "
...
specialist_agent = Agent(
    ...
    instruction=(
        ...
        "Check your inbox using specialist_check_inbox for escalations from front-desk. "
        ...
        "Also post a summary to the general channel using specialist_post_to_channel."
    ),
...
async def run_agent(agent: Agent, user_message: str, session_id: str) -> str:
    """Run a Google ADK agent with a message and return the final response."""
...
async def run_demo():
    print("=" * 60)
    print("Customer Service Escalation App — Google ADK")
    print("=" * 60)
    ...
    async with front_relay, specialist_relay:
        ...
        for customer_name, query in customer_queries:
            ...
            result = await run_agent(front_desk_agent, prompt, f"triage-{customer_name}")
        ...
        result = await run_agent(specialist_agent, prompt, "specialist-processing")
        ...
        result = await run_agent(front_desk_agent, prompt, "followup")
